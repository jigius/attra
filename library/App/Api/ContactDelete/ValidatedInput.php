...
final class ValidatedInput implements I\Api\ValidatableInputInterface
{
    ...
    public function validated(): I\Api\ValidatableInputInterface
    {
        ...
        if (!$body->has(EndpointInterface::BODY_PARAM_ID)) {
            throw
                new InvalidArgumentException(
                    "`" . EndpointInterface::BODY_PARAM_ID . "' is not defined",
                    400
                );
        }
        if (
            !is_numeric($body->param(EndpointInterface::BODY_PARAM_ID)) ||
            (int)$body->param(EndpointInterface::BODY_PARAM_ID) <= 0
        ) {
            throw
                new InvalidArgumentException(
                    "`" . EndpointInterface::BODY_PARAM_ID . "' is invalid",
                    400
                );
        }
        return $this;
